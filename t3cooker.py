...
=================================================================
	  ...
	  for jobid in range(1,self.njobs+1):
		  ...
		  fjobtemplate = open(params['job_template'],"r")
	        ...
	        job_replace_dic={}
	        if params['type_of_executable']=='cmssw' :
		  job_replace_dic={'MY_SCRAM_ARCH':my_scram_arch,'MY_CMSSW_BASE':str(cmssw_base_dir),
	 ...
	 command,
	  ...
	  for job in  statusdict.dict:
		  if os.path.exists(self._datasetDir+"/log/done/"+str(job)):
			  ...
			  continue
		  elif statusdict.dict[job]["ID"].split('.')[0] in qstat_list:
			  statusdict.dict[job]["status"]=qstat_list[statusdict.dict[job]["ID"].split('.')[0]]
	  ...
	  print 			   "Dataset        : " + bold + job_nice_name + reset ;
				    
	  #if len(done_jobs)   >0 : print "Done    : " + self.printCrabLike(done_jobs)
	  #if len(running_jobs)>0 : print "Running : " + self.printCrabLike(running_jobs)
	  #if len(queued_jobs)>0  : print "Queued  : " + self.printCrabLike(queued_jobs)
	  #if len(aborted_jobs)>0 : print "Aborted : " + self.printCrabLike(aborted_jobs)
	  #if len(created_jobs)>0 : print "Created : " + self.printCrabLike(created_jobs)
	  #if len(failed_jobs) >0 : print "Failed  : " + self.printCrabLike(failed_jobs)
	  #if len(killed_jobs) >0 : print "Killed  : " + self.printCrabLike(killed_jobs)
	  
	  
	  if len(done_jobs)   >0 : print 'Done    [%4d] : %s' % (len(done_jobs),self.printCrabLike(done_jobs))
	  if len(running_jobs)>0 : print 'Running [%4d] : %s' % (len(running_jobs),self.printCrabLike(running_jobs))
	  if len(queued_jobs)>0  : print 'Queued  [%4d] : %s' % (len(queued_jobs),self.printCrabLike(queued_jobs))
	  if len(aborted_jobs)>0 : print 'Aborted [%4d] : %s' % (len(aborted_jobs),self.printCrabLike(aborted_jobs))
	  if len(created_jobs)>0 : print 'Created [%4d] : %s' % (len(created_jobs),self.printCrabLike(created_jobs))
	  if len(failed_jobs) >0 : print 'Failed  [%4d] : %s' % (len(failed_jobs),self.printCrabLike(failed_jobs))
	  if len(killed_jobs) >0 : print 'Killed  [%4d] : %s' % (len(killed_jobs),self.printCrabLike(killed_jobs))
	  	    
	  remote_dir = str(self.configDict['user_remote_dir'])
	  
	  print 			   "Storage        :  " + remote_dir
